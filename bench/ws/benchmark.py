...
_TEXT = (
    ...
    "and sphinx of black quartz judges my vow; pack my box with five dozen liquor jugs. "
) * 4


def _message(idx):
    # a realistic payload: a chat-like JSON event of about 1 KB, sent as a text frame
    return json.dumps(
        {
            "id": idx,
            "type": "message",
            ...
            "user": {
                "id": idx % 1000,
                "name": f"user-{idx % 1000}",
                "role": "member",
                "avatar": f"https://example.org/avatars/{idx % 1000}.png",
            },
            "payload": {
                "text": _TEXT,
                "tags": ["alpha", "beta", "gamma", "delta"],
                "mentions": [idx % 7, idx % 11, idx % 13],
                "attachments": [],
            },
            "meta": {"client": "bench", "version": "1.0", "seq": idx, "priority": "normal"},
        },
        separators=(",", ":"),
    )
...
async def client(idx, messages, ready_signal):
    ...
    # no compression (not every server negotiates it) and no client keepalive: a long run
    # under a saturated client loop must not be cut by the client's own ping timeout
    async with websockets.connect("ws://127.0.0.1:8000", compression=None, ping_interval=None) as ws:
        ready_signal()
        await ready.wait()
...
async def benchmark(run):
    ...
    messages = [_message(idx) for idx in range(MSG_NO)]
